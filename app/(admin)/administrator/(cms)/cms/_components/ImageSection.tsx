...
import { ImageSectionProps } from "../types/form/image-section.types";
...
const ImageSection = ({
  ...
  const entityName = type === 'category' ? 'Category' : 'Article'

    const {formData,editingId, isSubmitting, isUploading} = storeData;
  ...
  return (
    <div className="mb-6 bg-gray-50 p-4 rounded border border-gray-200">
      <h3 className="text-lg font-medium mb-4">{entityName} image</h3>
            ...
            <div className="flex-1 mt-8">
              <p className="text-sm text-gray-600 mb-2">
                {formData.image.startsWith("blob:")
                  ? `New image (will be uploaded after saving) of ${entityName}`
                  : `Current image of ${entityName}`}
              </p>
              {formData.image.startsWith("blob:") && (
