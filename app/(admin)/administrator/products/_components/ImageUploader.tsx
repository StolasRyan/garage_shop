...
const ImageUploader = ({
  ...
  const convertToJpeg = useCallback(async (file: File): Promise<File> => {
    return new Promise((resolve, reject) => {
      ...
      img.onerror = () => reject(new Error("Failed convert image"));
  ...
  const handleFile = useCallback(
    async (file: File) => {
      ...
      if (file.size > maxSize) {
        return setError(
          `File size is too large! Max size: ${maxSize / 1024 / 1024} MB`,
        );
      }
      ...
      >
        ...
        <div className="space-y-2">
          ...
          <p className="text-sm text-gray-600">
            {converting ? "Converting..." : "Drag and drop or click to upload"}
          </p>
          <p className="text-sm text-gray-500">
            {converting ? 'Please wait...' : `Up to ${maxSize / 1024 / 1024} MB`}
          </p>
        </div>
      </div>
