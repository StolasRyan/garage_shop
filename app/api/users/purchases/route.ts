...
export async function GET(request: Request) {
    try{
       ...
       const purchases = await db.collection("products").find({id: {$in: productIds}}).sort({_id: -1}).skip(startIndex).limit(perPage).toArray();
       return NextResponse.json({products: purchases.map((product)=>{
        const{discountPercent, ...rest} = product;
        void discountPercent;
        return rest;
       }), totalCount}); 
