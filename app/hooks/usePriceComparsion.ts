...
export const usePriceComparsion = (
  order: Order,
  productsData: ProductCardProps[],
    ...
    const currentProducts = useMemo(():CurrentProduct[]=>{
        ...
        return order.items.map((item)=>{
            const productData = productsData.find((product)=>product.id.toString() === item.productId.toString());
    ...
    },[hasLoyaltyCard, order.items, productsData]);
